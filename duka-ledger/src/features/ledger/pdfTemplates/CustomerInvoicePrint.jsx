...
export default function CustomerInvoicePrint({ invoiceData, customerName, transactions }) {
  ...
  const finalCustomerName = invoiceData?.customerName || customerName;
  const invoiceNumber = invoiceData?.invoiceNumber || `INV-${String(finalCustomerName || '000').replace(/\s+/g, '').slice(0, 6).toUpperCase()}-${String(invoiceLines.length).padStart(3, '0')}`;
    ...
    <div className="ledger-invoice-grid space-y-6 text-slate-800 print:text-black font-sans p-6 bg-white border border-slate-200/80 rounded-2xl print:border-none print:p-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pb-6 border-b border-slate-200">
        <div>
          ...
          <h4 className="text-2xl font-black text-primary mt-1">CUSTOMER STATEMENT</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
        <div className="bg-slate-50/50 border border-slate-200/60 p-5 rounded-xl space-y-2">
          ...
          <h4 className="text-lg font-extrabold text-primary">{finalCustomerName}</h4>
          <div className="text-xs text-slate-600 space-y-1">
            <div><span className="font-semibold text-slate-400">Phone:</span> {summary.phone || invoiceData?.customerPhone || 'No phone verified'}</div>
            <div><span className="font-semibold text-slate-400">Reference:</span> {invoiceData?.customerLabel || 'Customer credit statement'}</div>
            {invoiceData?.referenceNote && (
              <div><span className="font-semibold text-slate-400">Notes:</span> {invoiceData.referenceNote}</div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-slate-50 border border-slate-150 p-3.5 rounded-xl flex flex-col justify-between">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Total Owed</span>
          ...
          <div className="bg-emerald-50/40 border border-emerald-100/80 p-3.5 rounded-xl flex flex-col justify-between">
            <span className="text-[9px] font-bold text-emerald-600 uppercase tracking-wider block">Total Repaid</span>
          ...
          <div className="bg-rose-50/50 border border-rose-100/80 p-3.5 rounded-xl flex flex-col justify-between">
            <span className="text-[9px] font-bold text-rose-600 uppercase tracking-wider block">Balance Due</span>
              ...
              return (
                <tr key={line.id} className="align-top odd:bg-slate-50/20">
                  ...
                  <td className="py-3 px-4 text-slate-800">
                    <div>{line.notes || (isDebt ? 'Debt entry' : 'Repayment entry')}</div>
                    {isDebt && line.item_name && (
                      <div className="text-[10px] text-slate-400 font-bold mt-0.5">Item: {line.item_name}</div>
                    )}
                  </td>
      ...
      <div className="bg-slate-50 border border-slate-150 p-4 rounded-xl text-[11px] text-slate-500 leading-relaxed">
        ...
        <p>{invoiceData?.footerNote || 'This statement reflects the customer ledger balances recorded in DukaLedger at the time of printing.'}</p>
      </div>
